...
var whitelist = ingredient.matchAny([
  ...
  events.listen('server.load', function (event) {
    ...
    event.server.tell([ text.lightPurple('[ServerKeeper]'), ' Timer started, clearing lag in 30 minutes!' ])
    // Schedule new task in 30 minutes
    event.server.schedule(MINUTE * 10, event.server, function (callback) {
      // Tell everyone on server that items will be removed
      callback.data.tell([ text.lightPurple('[ServerKeeper]'), ' Removing all items on ground in one minute!' ])
  ...
  events.listen('command.registry', function (event) {
    ...
    event
      ...
      .execute(function (sender, args) {
        sender.tell([ text.lightPurple('[ServerKeeper]'), ' Last clearlag results:' ])
